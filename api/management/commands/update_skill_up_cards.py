...
def update_skill_up_cards(opt={}):
    ...
    for skill in all_skills:
        skills[skill] = ','.join([str(card['id']) + '-' + card['name'].split(' ')[-1] for card in models.Card.objects.filter(is_promo=False, skill=skill).exclude(idol__main_unit='A-RISE').values('id', 'name')])
    print skills
